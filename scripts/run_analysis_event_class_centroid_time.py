...
#Run regression
if run_regression:
    Ai,ki,mcmci=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='i',dependent_variable='centroid_time')
    Au,ku,mcmcu=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='u',dependent_variable='centroid_time')
    Al,kl,mcmcl=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='l',dependent_variable='centroid_time')
    Ana,kna,mcmcna=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='n/a',dependent_variable='centroid_time')
else:
    A=9.05037048766e-06
    k=0.279444862572
...
plt.plot(Mo,Td,'--k',lw=2)
plt.legend(['interplate','lower','upper','n/a','Ye et al. 16'],loc=2)
