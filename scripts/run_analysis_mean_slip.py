...
#Run regression
if run_regression:
    A,k,mcmc=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=1000e3,burn=100e3,fix_exponent=False,dependent_variable='slip')
    print '\n'
    print A
    print k
else:
...
plt.scatter(neic.moments[i],neic.mean_slip[i],c='#1E90FF',marker='D',s=40)

i=where(neic.event_type=='N')[0]
plt.scatter(neic.moments[i],neic.mean_slip[i],c='#DC143C',marker='s',s=40)

i=where(neic.event_type=='T')[0]
plt.scatter(neic.moments[i],neic.mean_slip[i],c='#FFD700',marker='^',s=40)
...
yticks = [0.2,0.5,1.0,2.0,5.0,10,20,50]
labels = yticks
...
plt.scatter(neic.moments[i],neic.mean_slip[i],c='#87CEFA',marker='D',s=40)
...
plt.scatter(neic.moments[i],neic.mean_slip[i],c='#228B22',marker='o',s=40)
...
plt.scatter(neic.moments,neic.mean_slip,c='#1E90FF',marker='D',s=40)
Dx=0.05
Dy=0.002
for k in range(len(neic.moments)):
    dx=neic.moments[k]*Dx
    dy=neic.event_durations[k]*Dy
    plt.annotate(xy=(neic.moments[k]+dx,neic.mean_slip[k]+dy),s=neic.names[k],fontsize=10)


plt.ylim([0.2,70])
plt.xlim([1e19,1e23])
plt.xlabel('Moment (Nm)',fontsize=14)
plt.ylabel('Mean_slip (m)',fontsize=14)
