...
#Run regression
if run_regression:
    Ai,ki,mcmci=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='i',dependent_variable='duration')
    Au,ku,mcmcu=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='u',dependent_variable='duration')
    Al,kl,mcmcl=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='l',dependent_variable='duration')
    Ana,kna,mcmcna=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=100e3,burn=20e3,fix_exponent=False,select_event_type='n/a',dependent_variable='duration')
else:
    A=9.05037048766e-06
    k=0.279444862572
...
plt.scatter(neic.moments[i],neic.event_durations[i],c='#000080',marker='D',s=40,zorder=90)
i=where(neic.event_class=='l')[0]
plt.scatter(neic.moments[i],neic.event_durations[i],c='#FFD700',marker='^',s=40,zorder=90)
i=where(neic.event_class=='u')[0]
plt.scatter(neic.moments[i],neic.event_durations[i],c='#DC143C',marker='s',s=40,zorder=90)
i=where(neic.event_class=='n/a')[0]
plt.scatter(neic.moments[i],neic.event_durations[i],c='#228B22',marker='o',s=40,zorder=90)
