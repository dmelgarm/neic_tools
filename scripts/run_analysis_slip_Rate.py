...
#Run regression
if run_regression:
    A,k,mcmc=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=1000e3,burn=100e3,fix_exponent=False,dependent_variable='slip_rate')
    print '\n'
    print A
    print k
else:
...
plt.scatter(neic.moments[i],neic.mean_slip_rates[i],c='#1E90FF',marker='D',s=40)
...
plt.xlim([1e19,1e23])
plt.xlabel('Moment (Nm)',fontsize=14)
plt.ylabel('Mean slip_rate (m/s)',fontsize=14)
...
plt.scatter(neic.moments,neic.mean_slip_rates,c='#1E90FF',marker='D',s=40)
Dx=0.05
Dy=0.05
for k in range(len(neic.moments)):
    dx=neic.moments[k]*Dx
    dy=neic.mean_slip_rates[k]*Dy
    plt.annotate(xy=(neic.moments[k]+dx,neic.mean_slip_rates[k]+dy),s=neic.names[k],fontsize=10)
