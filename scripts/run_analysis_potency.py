...
#Run regression
if run_regression:
    A,k,mcmc=neic.run_regression(inversion_type='bayesian',prior='uninformative',Niter=1000e3,burn=100e3,fix_exponent=False,dependent_variable='potency')
    print '\n'
    print A
    print k
else:
...
ax.annotate(r'$t_r=A M_0^k$',xy=(-5.03,0.43),fontsize=17)
ax.annotate('freq.',xy=(-7.6,0.41),fontsize=14)
...
plt.scatter(neic.moments[i],neic.mean_potency[i],c='#1E90FF',marker='D',s=40)

i=where(neic.event_type=='N')[0]
plt.scatter(neic.moments[i],neic.mean_potency[i],c='#DC143C',marker='s',s=40)

i=where(neic.event_type=='T')[0]
plt.scatter(neic.moments[i],neic.mean_potency[i],c='#FFD700',marker='^',s=40)
...
plt.ylim([10**6.5,10**10.5])
plt.xlim([1e19,1e23])
plt.xlabel('Moment (Nm)',fontsize=14)
plt.ylabel('Mean potency (m^3)',fontsize=14)

#yticks = [1e7, 1e8,1e9,1e10]
#labels = [r'$10^7$',r'$10^8$',r'$10^9$',r'$10^{10}$']
...
plt.scatter(neic.moments[i],neic.mean_potency[i],c='#87CEFA',marker='D',s=40)
...
plt.scatter(neic.moments[i],neic.mean_potency[i],c='#228B22',marker='o',s=40)
...
plt.scatter(neic.moments,neic.mean_potency,c='#1E90FF',marker='D',s=40)
Dx=0.05
Dy=0.05
for k in range(len(neic.moments)):
    dx=neic.moments[k]*Dx
    dy=neic.mean_potency[k]*Dy
    plt.annotate(xy=(neic.moments[k]+dx,neic.mean_potency[k]+dy),s=neic.names[k],fontsize=10)
